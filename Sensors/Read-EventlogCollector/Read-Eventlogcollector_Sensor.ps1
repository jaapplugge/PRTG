<#
.SYNOPSIS
PRTG-Sensor for reading Eventlogs from an EventlogCollector.

.DESCRIPTION
This script is ment to be used as PRTG-sensor. It will read the Forwarded Events on a remote
...
This sensor will not return the number of computers who pushed a certain events to the Forwarded Events eventlog. 
Instead, it will return the number of occurences of an event, even if it occured twice or more on a single computer. 
(this is the difference between this sensor and the Read-EventlogCollectorUnique_sensor)
...
- EXE/Script:       read-eventlogcollector_sensor.ps1
- Parameters:       '-Filename "\\Server.domain.local\share\folder\read-eventlogcollector_channelConfiguration.xml" -Computer "plugge-fs01.plugge-d.local" '
...
.\Read-Eventlogcollector_sensor.ps1 -Filename "\\Server.domain.local\share\folder\read-eventlogcollector_channelConfiguration.xml" -Computer "plugge-fs01.plugge-d.local"
    ...
    Foreach ($Channel in $ChannelConfiguration) {
        ...
        If ($Boolean_Skip -eq $False) {
            If ($Boolean_Cred -eq $False) {
                Try {
                    $ReturnValue = Get-PRTGClientEventlog -Computer $Server -Query $Query -Eventlog $Eventlog
                    Write-Verbose "$Timestamp : LOG   : Queried remote eventlog. (Return: $($ReturnValue.Count) events)"
                } Catch {
                    Write-Error     "$TimeStamp : ERROR : Could not Query remote eventlog for server $Server."
                    Write-Error     "$TimeStamp : ERROR : $($_.Exception.Message)"
                    $Output_Message = "Could not Query remote eventlog for server $Server."
                    $Boolean_Skip = $True
                    $Boolean_Exit = $True
                }
            } Else {
                Try {
                    ...
                    Write-Error     "$TimeStamp : ERROR : $($_.Exception.Message)"
                    $Output_Message = "Could not Query remote eventlog for server $Server w\ cred."
                    $Boolean_Skip = $True
                    $Boolean_Exit = $True
                }
            }
