<#
.SYNOPSIS Get-SCCMSCEPDefinitionStatus
...
- EXE/Script:       Get-SCCMSCEPDefinitionStatus.ps1
- Parameters:       -SiteServer 'Sccm01.ogd.local' -Site 'OGD' -Configuration 'C:\Scripting\Configurations\Get-SCCMSCEPDefinitionStatus_ChannelConfiguration.xml'
...
If ($Boolean_Exit -eq $false) {
    Try { 
        ...
        $Configuration = Write-PRTGresult -Configuration $Configuration -Channel "All Clients without SCEPClient" -Value $( ($Collected.TotalMemberCount)-($Collected.ApplyPolicySucceededCount) )
        Write-Verbose    "$TimeStamp : LOG   : All Clients without SCEPClient   : $( ($Collected.TotalMemberCount)-($Collected.ApplyPolicySucceededCount) )"
        $Configuration = Write-PRTGresult -Configuration $Configuration -Channel "All Clients without Definitions" -Value $($Collected.NoSignatureCount)
        Write-Verbose    "$TimeStamp : LOG   : All Clients without Definitions  : $($Collected.NoSignatureCount)"
        $Configuration = Write-PRTGresult -Configuration $Configuration -Channel "Definitions older than 7 days" -Value $($Collected.SignaturesOlderThan7DaysCount)
        Write-Verbose    "$TimeStamp : LOG   : Definitions older than 7 days    : $($Collected.SignaturesOlderThan7DaysCount)"
        $Configuration = Write-PRTGresult -Configuration $Configuration -Channel "Definitions less than 7 days old" -Value $($Collected.SignaturesUpTo7DaysOldCount)
        Write-Verbose    "$TimeStamp : LOG   : Definitions less than 7 days old : $($Collected.SignaturesUpTo7DaysOldCount)"
        $Configuration = Write-PRTGresult -Configuration $Configuration -Channel "Definitions less than 3 days old" -Value $($Collected.SignaturesUpTo3DaysOldCount)
        Write-Verbose    "$TimeStamp : LOG   : Definitions less than 3 days old : $($Collected.SignaturesUpTo3DaysOldCount)"
        $Configuration = Write-PRTGresult -Configuration $Configuration -Channel "Definitions less than 1 days old" -Value $($Collected.SignaturesUpTo1DayOldCount)
        Write-Verbose    "$TimeStamp : LOG   : Definitions less than 1 days old : $($Collected.SignaturesUpTo1DayOldCount)"       
    } Catch {
